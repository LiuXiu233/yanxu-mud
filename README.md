...
当前版本：**0.4.1（持久化预览）**。本版本提供可实际使用的世界核心、内容工具和生产 SQLite 持久化；公共 API 在 1.0 前仍可能调整。玩法系统和网络接入仍在后续阶段，本文不会将它们描述为已经可用。
...
`examples/青石镇/内容` 是与引擎分离、可检查和可打包的示例世界内容。示例包版本保留为 0.3.0，用于持续验证内容向后兼容。0.4.1 尚未包含账户、角色和玩法运行时，因此还不能进入青石镇游玩。
