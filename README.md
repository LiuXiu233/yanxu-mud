...
最新发布版本：**0.7.0（网络接入预览）**。独立的青石镇游戏包已接通完整基础玩法、本地控制台、Telnet、WebSocket、HTTP 管理接口和响应式浏览器客户端；连接、会话、账户、角色与实体保持独立，支持恢复令牌、限流和有界输出队列。公共 API 在 1.0 前仍可能调整。
...
- 持久 AI 组件、可替换行为、稳定编号目标选择，以及攻击效果、行为状态和任务重排同事务提交的确定性行动。
- 稳定频道定义、持久订阅与静音、附近/世界/私信路由，以及带确定性接收者元数据的结构化聊天消息。
...
- 纯文本与 ANSI 本地控制台，以及统一的 `运行/run`、`控制台/console` CLI，可按顺序执行命令并持久保存游戏状态。
- 连接与会话分离、HMAC-SHA-256 恢复令牌轮换、角色独占、会话保留、输入限流、有界输出队列和无秘密审计投影。
- UTF-8 Telnet 行协议、NAWS/字符集协商、多字节退格、NVT 输出，以及带读取、协商、输入和背压预算的真实 TCP 服务器。
- RFC 6455 握手、掩码帧、分片、Ping/Pong、关闭、UTF-8 与消息上限，以及保留升级后首帧字节的真实 WebSocket 服务器。
- 健康、服务器、在线、登录、内容、热重载、快照、日志与诊断 HTTP 端点，以及带 CSP、`no-store` 和路径穿越防护的静态客户端。
- 青石镇浏览器客户端支持直接 WebSocket 登录、游客进入、恢复令牌重连、命令历史、安全 DOM 渲染、地图和响应式状态视图。
...
在默认执行预算下运行青石镇并观察起始场景：

```bash
yanxu 包 运行 . -- 运行 --命令 观察 --存档 .yanxu/青石镇存档.json examples/青石镇
```
...
`examples/青石镇` 是与引擎分离的 0.7.0 游戏包；其言据内容、言序规则、运行时分片、浏览器资源、测试和存档入口均由自己的清单管理。玩家可以移动和观察，管理物品与装备，查看状态和技能，战斗、复活、接取任务、交谈、拾取掉落、聊天、保存与恢复；管理员命令可生成物品和检查刷新状态。

`运行/run` 与 `控制台/console` 都支持 `--模式 控制台|Telnet|WebSocket|HTTP`。控制台模式接受重复 `--命令`；服务器模式接受 `--地址` 和用于测试的 `--次数`，零表示持续运行。言序 1.1.12 的任务是单线程协作式执行，因此三个监听器使用三个独立进程，而不是在一个进程中假装并行。浏览器直接向 WebSocket 进程登录或进入游客模式，HTTP 进程只提供静态资源和管理端点。
...
浏览器访问 `http://127.0.0.1:8080/`。游客可以直接进入；环境变量 `YANYU_LOGIN_NAME` 与 `YANYU_LOGIN_SECRET` 同时存在时启用登录。`YANYU_SESSION_SECRET` 至少 32 字节，用于跨重启保留恢复令牌；未配置时使用仅本次进程有效的安全随机密钥。

Telnet 默认监听 `127.0.0.1:4000`：
...
连接后输入 `游客`，或输入 `登录 <名称> <密钥>`。同一存档路径只能由一个可写玩法进程使用；示例轻量存档没有跨进程锁。完整协议与生产边界见 [Telnet](docs/TELNET.md)、[WebSocket](docs/WEBSOCKET.md)、[Web 与管理接口](docs/WEB_ADMIN.md)和[部署](docs/DEPLOYMENT.md)。
...
| 0.4.1 | 已发布 | 干净克隆锁定与跨平台 SQLite 制品校验修复 |
| 0.5.0 | 已发布 | 账户角色、权限、完整基础玩法、频道与本地控制台 |
| 0.7.0 | 当前 | Telnet、WebSocket、HTTP、会话恢复、背压与浏览器客户端 |
