...
最新发布版本：**0.5.0（完整基础玩法预览）**。独立的青石镇游戏包已接通账户、角色、权限、空间、物品、背包、装备、属性、状态、冷却、技能、战斗、死亡、任务、对话、掉落、时间、刷新、AI、频道和本地控制台；Telnet、WebSocket、HTTP 与浏览器客户端将在 0.7.0 提供。公共 API 在 1.0 前仍可能调整。
...
`examples/青石镇` 是与引擎分离的 0.5.0 游戏包；其言据内容、言序规则、运行时分片、测试和存档入口均由自己的清单管理。玩家可以移动和观察，管理物品与装备，查看状态和技能，战斗、复活、接取任务、交谈、拾取掉落、聊天、保存与恢复；管理员命令可生成物品和检查刷新状态。

`运行/run` 与 `控制台/console` 当前都启动有界的本地控制台脚本。重复 `--命令` 可顺序执行多条命令，`--json` 返回结构化报告，`--ansi` 使用终端渲染，`--存档` 指定游戏目录内的轻量存档。言序 1.1.12 没有持久标准输入进程接口，因此根 CLI 不伪装成交互式子进程；直接游戏包同样接受命令列并由本地控制台执行。当前版本没有可用的 Telnet 端口或 Web 地址。
...
| 0.5.0 | 当前 | 账户角色、权限、完整基础玩法、频道与本地控制台 |
